...
# if(kruskal.test(DOY~size,data=subset(input,percentile=="GS_beg"))$p.value<0.05) g<-g+annotate("text",label="*",y=250,size=8,colour=col,x=1)
  ...
  if(summary(lmer(DOY    ~ size + (1|Tree) + (1|Year), data = subset(input,percentile=="GS_beg")))$coefficients[,"Pr(>|t|)"][2]<0.05) g<-g+annotate("text",label="*",y=250,size=8,colour=col,x=1)
  if(summary(lmer(DOY    ~ size + (1|Tree) + (1|Year), data = subset(input,percentile=="25th")))$coefficients[,"Pr(>|t|)"][2]<0.05) g<-g+annotate("text",label="*",y=250,size=8,colour=col,x=2)
  if(summary(lmer(DOY    ~ size + (1|Tree) + (1|Year), data = subset(input,percentile=="50th")))$coefficients[,"Pr(>|t|)"][2]<0.05) g<-g+annotate("text",label="*",y=250,size=8,colour=col,x=3)
  if(summary(lmer(DOY    ~ size + (1|Tree) + (1|Year), data = subset(input,percentile=="75th")))$coefficients[,"Pr(>|t|)"][2]<0.05) g<-g+annotate("text",label="*",y=250,size=8,colour=col,x=4)
  if(summary(lmer(DOY    ~ size + (1|Tree) + (1|Year), data = subset(input,percentile=="GS_end")))$coefficients[,"Pr(>|t|)"][2]<0.05) g<-g+annotate("text",label="*",y=250,size=8,colour=col,x=4)
