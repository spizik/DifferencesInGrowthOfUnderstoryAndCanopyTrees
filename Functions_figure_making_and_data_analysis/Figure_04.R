...
test.differences.size.category<-function(site){
  ...
  output_big<-as.data.frame(dunn.test(input$Rel_periods, input$DayPhaseSp, table = F))
  ...
  input$DayPhaseSp<-paste0(input$DayPhase, "_", input$Species)
  output_small<-as.data.frame(dunn.test(input$Rel_periods, input$DayPhaseSp, table = F))
...
test.differences.withinsize<-function(site, species){
  ...
  input<-subset(dataset,Site==site & Species==species & Size=="big")
  output_big<-as.data.frame(dunn.test(input$Rel_periods, input$DayPhase , table = F))
  output_big<-output_big[,c("comparisons", "P.adjusted")]
  names(output_big)<-c("Comparison", "pval")
  output_big$pval<-round(output_big$pval, 3)
  output_big$size<-"big"
  output_big$spp<-species
  output_big$site<-site
  
  input<-subset(dataset,Site==site & Size=="small")
  output_small<-as.data.frame(dunn.test(input$Rel_periods, input$DayPhase, table = F))
  output_small<-output_small[,c("comparisons", "P.adjusted")]
  names(output_small)<-c("Comparison", "pval")
  output_small$pval<-round(output_small$pval, 3)
  output_small$size<-"small"
  output_small$spp<-species
  output_small$site<-site
