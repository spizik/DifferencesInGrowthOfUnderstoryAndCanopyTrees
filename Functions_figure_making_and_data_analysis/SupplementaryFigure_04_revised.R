...
test.differences_nogro_gro<-function(site, species, test="lmer"){
  ...
  if(test == "lmer"){
    gr.hours <- summary(lmer(Rate_NoGRO_GRO   ~ Size + (1|Tree) + (1|Year), data = input))
...
test.differences_twd_gro<-function(site, species, test="lmer"){
  ...
  if(test == "lmer"){
    gr.hours <- summary(lmer(Rate_TWD_GRO   ~ Size + (1|Tree) + (1|Year), data = input))
...
test.differences_refiling_gro<-function(site, species, test="lmer"){
  ...
  if(test == "lmer"){
    gr.hours <- summary(lmer(Rate_Refiling_GRO   ~ Size + (1|Tree) + (1|Year), data = input))
...
## test.differences_refiling_gro ####
# Test size-related differences in the proportion of periods with no change in stem size relative to all periods.
test.differences_nochange<-function(site, species, test="lmer"){
  ...
  if(test=="anova"){
    gr.hours<-summary(aov( Rate_noChange~Size,data=input))
    
    gr.hours<-round(gr[[1]][["Pr(>F)"]][1],3)
  }
  if(test=="kw-test"){
    gr.hours<-kruskal.test( Rate_noChange~Size,data=input)
    
    gr.hours<-round(gr.hours$p.value,3)
  }
  if(test=="wilcox"){
    gr.hours<-wilcox.test( Rate_noChange~Size,data=input)
    
    gr.hours<-round(gr$p.value,3)
  }
  if(test == "lmer"){
    gr.hours <- summary(lmer( Rate_noChange   ~ Size + (1|Tree) + (1|Year), data = input))
...
differences_nochange<-rbind(test.differences_nochange("Ranspurk","ACCA"),
                                test.differences_nochange("Ranspurk","CABE"),
                                test.differences_nochange("Ranspurk","ULSP"),
                                test.differences_nochange("Zofin","PCAB"),
                                test.differences_nochange("Zofin","FASY"),
                                test.differences_nochange("Boubin","PCAB"),
                                test.differences_nochange("Boubin","FASY"),
                                test.differences_nochange("Eustaska","PCAB"))
