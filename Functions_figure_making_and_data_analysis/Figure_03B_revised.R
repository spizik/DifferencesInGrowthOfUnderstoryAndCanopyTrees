...
test.differences<-function(site, species, test="lmer"){
  ...
  if(test=="lmer"){
    ...
    all <- summary(lmer(rel.incr   ~ Size + (1|Tree) + (1|Year), data = input))
    sunrise <- summary(lmer(rel.incr   ~ Size + (1|Tree) + (1|Year), data = input))
    day <- summary(lmer(rel.incr   ~ Size + (1|Tree) + (1|Year), data = input))
    sunset <- summary(lmer(rel.incr   ~ Size + (1|Tree) + (1|Year), data = input))
    night <- summary(lmer(rel.incr   ~ Size + (1|Tree) + (1|Year), data = input))
    
    all<-round(all$coefficients[,"Pr(>|t|)"][2],3)
